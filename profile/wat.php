...
<section>
    <?php  
    $stmt="SELECT DISTINCT imdbid,movname,poster FROM watched WHERE user_uid = '$uid'";
    $result=mysqli_query($con,$stmt);
    $resultCheck = mysqli_num_rows($result);

    if($resultCheck>0){
        echo "<h2> WATCHED ..:)</h2>";
    $sql=$con->prepare("SELECT DISTINCT imdbid,movname,poster FROM watched WHERE user_uid = '$uid'");
    $sql->execute();
